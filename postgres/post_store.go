...
func (s *PostStore) Post(id uuid.UUID) (miji.Post, error) {
	var p miji.Post
	if err := s.Get(&p, `SELECT * FROM posts WHERE id = $1`, id); err != nil {
		return miji.Post{}, fmt.Errorf("error getting post: %w", err)
	}

	return p, nil
}

func (s *PostStore) PostsByThread(threadID uuid.UUID) ([]miji.Post, error) {
	var pp []miji.Post

	if err := s.Select(&pp, `SELECT * FROM posts WHERE thread_id = $1`, threadID); err != nil {
		return []miji.Post{}, fmt.Errorf("error getting posts: %w", err)
	}

	return pp, nil
}

func (s *PostStore) CreatePost(p *miji.Post) error {
...
func (s *PostStore) UpdatePost(p *miji.Post) error {
